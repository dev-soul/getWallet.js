$(document).ready(function () {
  ...
  if (localStorage.getItem("sessionExpired") === null) {
    var expiredDate = Math.floor(Date.now() / 1000)
    expiredDate = expiredDate + 1800000
    localStorage.setItem("sessionExpired", expiredDate);
    }
  
    if (parseInt(localStorage.getItem("sessionExpired")) < now){
  ...
  if (
    localStorage.getItem("firstRun") === 'false' && parseInt(localStorage.getItem("sessionExpired")) > now
  ) {
    getAddress();
  }
